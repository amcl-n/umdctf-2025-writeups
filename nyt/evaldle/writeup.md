...
`SIGMA`という文字列を正解としたwordleになっている。flagに関わる部分の挙動を要約すると、「標準入力で入れた5文字（以内）の文字列を`exec()`で評価するので、それを利用して変数`f`にstrとして格納されているflagを探れ」ということになる。
...
ここまでくれば、後はflag文字列を前から順に総当たり的に決定していくスクリプトをゴリゴリ作っていけばOK。

[main.py](./main.py)

1回の評価のために何回もリクエストを送る必要があるのでやや時間がかかるが、最終的に以下のようなflagが得られる。
